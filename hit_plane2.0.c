...
void menu(void);//菜单
int scr[22][N]={0},pl=9,width=24,speed=3,density=30,score=0,death=0;//全局变量：界面、我机初始位、界面宽度、敌机速度、敌机密度、得分、死亡，可以修改
main(void)
{
	...
	movebul(scr);
	print(scr);
	if(i==30000)i=0;//以免i 越界,我觉得可以试试修改看看效果。。。
	}
}
void print(int a[][N])
{
	...
	for(j=0;j<width;j++)
	{
		...
		if(i==3&&j==width-1)printf("修改:Melo");
	}
	printf("\n");
}
}//麻蛋，不知道为啥这里有个括号就对了！
...
do
{
	...
	case '3':density=40;break;//为了格式也是拼了
	default:printf("\n 错误，请重新选择...\n");
		sw=1;
	}
}
...
void menu(void)
{
printf("注意了！按A D 控制我机左右飞行，W 发射子弹\n 设置：请按Esc\n 开始游戏：任意键\n                          --周庆小组专用游戏");
if(getch()==27)setting();
}